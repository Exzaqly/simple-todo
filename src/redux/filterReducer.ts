...
export const actions = {
  setFilterValue: (filter: ShowFilterType) =>
    ({ type: SET_FILTER_VALUE, payload: { filter } } as const),
  setSortingValue: (sort: SortingType) =>
    ({ type: SET_SORTING_VALUE, payload: { sort } } as const),
}

export const setFilterValue =
  (filter: ShowFilterType): Thunk =>
  (dispatch) => {
    dispatch(actions.setFilterValue(filter))
  }
export const setSortingValue =
  (sort: SortingType): Thunk =>
  (dispatch) => {
